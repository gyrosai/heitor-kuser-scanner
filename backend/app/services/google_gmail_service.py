...
def _plain_to_html(text: str) -> str:
    ...
    parts = [f"<p>{p.replace(chr(10), '<br>')}</p>" for p in paragraphs if p.strip()]
    return (
...
def _build_mime_message(
    ...
    """Monta o e-mail (plain + html), com ou sem anexo.

    Legado (mídia kit fixo, ``attachment_path`` informado):
        multipart/mixed
        ├── multipart/alternative
        │   ├── text/plain   (fallback)
        │   └── text/html    (preferido — RFC 2046: best last, auto-gerado
        │                      de body_text via _plain_to_html)
        └── application/pdf  (anexo)

    Pacote de materiais (sem anexo, ``body_html`` já pronto pelo chamador):
        multipart/alternative
        ├── text/plain
        └── text/html
    """
    html_body = body_html if body_html is not None else _plain_to_html(body_text)
...
async def send_via_gmail(
    ...
    Args:
        body_html: HTML já pronto (pacote de materiais). None = gera HTML
            automaticamente a partir de body_text (comportamento legado).
        attach_media_kit: se True (padrão, legado), anexa o PDF do mídia kit
            do idioma. Pacote de materiais chama com False (só links, sem
            anexo).
    """
